...
export default function FormContainer({
  ...
  const handleDisableNext = (values: {[key: string]: string}, errors: FormikErrors<{[key: string]: string}>) => {
    ...
    // Use short-circuit evaluation over all keys to find any empty value for the current category
    // This avoids creating an intermediate dictionary (getCurrentCategoryData) and exits early if found
    return Object.keys(values).some(key => key.startsWith(currentCategory.slug) && values[key] === "");
  };
