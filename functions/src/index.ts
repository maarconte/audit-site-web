...
const allowedOrigins = defineString("ALLOWED_ORIGINS", { default: "http://localhost:3000" });
...
export const submitForm = onRequest(
	{ secrets: [brevoApiKey] },
	(req, res) => {
		// Use require to bypass any TypeScript esModuleInterop issues with cors package
		const corsHandler = require("cors")({
			origin: allowedOrigins.value().split(",").map((o: string) => o.trim()),
		});
