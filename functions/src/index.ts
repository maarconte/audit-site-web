...
export const submitForm = onRequest(
	...
	async (req, res) => {
		...
		// 🛡️ SECURITY: Strict validation for dynamic `scores` object to prevent Mass Assignment/NoSQL Injection
		if (scores !== undefined) {
			...
			for (const key of scoreKeys) {
				if (key.length > 50 || typeof scores[key] !== "number") {
					res.status(400).json({ success: false, message: "Valeurs des scores invalides." });
					return;
				}
			}
		}
