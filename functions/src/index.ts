...
export const submitForm = onRequest(
	...
	async (req, res) => {
		...
		// 🛡️ SECURITY: Validate scores payload to prevent Mass Assignment and NoSQL injection
		if (scores !== undefined) {
			...
			for (const key of scoreKeys) {
				...
				if (typeof val === "string" && val.length > 500) {
					res.status(400).json({ success: false, message: "Valeur de score trop longue." });
					return;
				}
			}
		}
